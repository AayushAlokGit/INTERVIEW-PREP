...
- **Label the problem's difficulty when you present it** — one line directly under the prompt: `**Difficulty: X/5**`. Take X from the problem pool; for a problem not in the pool, rate it yourself on the same scale (1 = trivial object model, obvious entities · 3 = a real rule to place and one genuine trade-off · 5 = subtle model that barely fits 40 minutes). **The rating alone — no explanation, no descriptor, no text of any kind after the `X/5`.** Nothing else about difficulty is said again during the round.
